...
- \`cod inline\` pentru variabile / funcții / operatori (ex: \`print()\`, \`x > 0\`)
- **bold** pentru cuvinte cheie / accente importante
- *italic* pentru sublinieri ușoare
- \`\`\`python ... \`\`\` pentru blocuri scurte de cod (max 5 linii)
...
Evaluează codul cu un barem și răspunde STRICT în JSON:
{
  ...
  "reasoning": "<2-4 propoziții PRIETENOASE pentru elev: ce a făcut bine, ce trebuie corectat, exemplu concret. Folosește markdown: \`cod\` inline pentru sintaxă (ex: \`print()\`, \`x > 0\`), **bold** pentru accente, blocuri \`\`\`python\`\`\` pentru exemple scurte. Maxim 400 caractere.>",
