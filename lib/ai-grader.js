...
- Cod pe MAI MULTE LINII sau cu indentare → OBLIGATORIU bloc \`\`\`limbaj ... \`\`\` pe linii SEPARATE, cu INDENTARE CORECTĂ (4 spații pentru Python, 2 pentru JS):
  \`\`\`python
  if x > 0:
      print("pozitiv")
  \`\`\`
...
Evaluează codul cu un barem și răspunde STRICT în JSON:
{
  ...
  "reasoning": "<2-4 propoziții PRIETENOASE pentru elev: ce a făcut bine, ce trebuie corectat, exemplu concret. REGULI FORMAT: cod inline scurt (variabile, funcții) cu backtick simplu ex: \`print()\`, \`x > 0\`. Cod pe MAI MULTE LINII (cu indentare, if/for/def etc.) OBLIGATORIU cu bloc \`\`\`${language} ... \`\`\` pe linii separate cu indentare corectă! NICIODATĂ nu pune cod multi-linie cu backtick simplu! **bold** pentru accente. Maxim 500 caractere.>",
