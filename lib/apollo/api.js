...
export async function adaugaInSecventa(secventaId, contactIds, mailboxIds) {
  ...
  // Apollo poate accepta cererea dar să NU adauge pe cineva — cel mai des
  // pentru că e deja în secvența asta sau în alta activă. Îi întoarce separat,
  // în `skipped_contact_ids`. Dacă nu citim câmpul ăsta, raportăm „0 adăugate"
  // fără să spunem de ce, iar omul crede că s-a stricat ceva.
  const adaugateIds = (r?.contacts || []).map((c) => c.id)
  const sariteIds = r?.skipped_contact_ids || []

  return {
    adaugate: adaugateIds.length,
    adaugateIds,
    sarite: sariteIds.length,
    sariteIds,
    // Cine n-a fost nici adăugat, nici raportat ca sărit — n-ar trebui să existe,
    // dar dacă apare vrem să știm, nu să-l pierdem în tăcere.
    necunoscuteIds: contactIds.filter(
      (id) => !adaugateIds.includes(id) && !sariteIds.includes(id)
    ),
  }
}

/**
 * Când pleacă primul email către contactele tocmai adăugate.
 *
 * Apollo nu trimite instant: pune mesajul la coadă în fereastra orară a
 * secvenței și respectă pauza dintre emailuri. Fără informația asta, omul
 * crede că nu s-a trimis nimic.
 */
export async function candPleacaPrimul(secventaId, contactIds) {
  try {
    const r = await apollo('/emailer_messages/search', {
      metoda: 'POST',
      corp: { emailer_campaign_ids: [secventaId], contact_ids: contactIds, per_page: 50 },
    })

    const programate = (r?.emailer_messages || [])
      .filter((m) => contactIds.includes(m.contact_id) && m.due_at && !m.completed_at)
      .map((m) => new Date(m.due_at))
      .filter((d) => !Number.isNaN(d.getTime()))
      .sort((a, b) => a - b)

    return programate[0] || null
  } catch {
    return null // informativ, nu blocant
  }
}
...
export async function statisticiSecventa(secventaId) {
  ...
  const stat = {
    ...
    // Pașii = follow-up-ul, fiecare cu textul lui și cu performanța lui
    pasi: (r?.emailer_steps || []).map((p) => {
      // Un pas → un „touch" (emailul) → un „template" (subiect + corp).
      const touch = (r?.emailer_touches || []).find((t) => t.emailer_step_id === p.id)
      const sablon = (r?.emailer_templates || []).find((t) => t.id === touch?.emailer_template_id)
      ...
      return {
        ...
        // Conținutul, ca să-l poți citi și schimba fără să intri în Apollo
        templateId: sablon?.id || null,
        subiect: sablon?.subject || null,
        corpHtml: sablon?.body_html || null,
        corpText: htmlInText(sablon?.body_html || ''),

        // Cum s-a descurcat exact acest email — util ca să vezi care pas prinde
        statistici: touch
          ? {
              livrate: touch.unique_delivered ?? 0,
              deschise: touch.unique_opened ?? 0,
              raspunsuri: touch.unique_replied ?? 0,
              bounce: touch.unique_bounced ?? 0,
              rataDeschidere: procent(touch.open_rate),
              rataRaspuns: procent(touch.reply_rate),
            }
          : null,
      }
    }),
 ...
 *
 * Modificarea afectează DOAR emailurile care pleacă de acum înainte — cele
 * trimise deja rămân cum au fost. Dacă secvența e activă, schimbarea intră în
 * vigoare imediat pentru contactele care n-au ajuns încă la pasul ăsta.
 */
export async function actualizeazaEmail(templateId, { subiect, corp }) {
  if (!templateId) throw new Error('Lipsește id-ul emailului')
  if (!subiect?.trim()) throw new Error('Subiectul nu poate fi gol')
  if (!corp?.trim()) throw new Error('Textul nu poate fi gol — Apollo refuză emailurile goale')

  const r = await apollo(`/emailer_templates/${templateId}`, {
    metoda: 'PUT',
    corp: {
      subject: subiect.trim(),
      body_html: textInHtml(corp),
    },
  })

  return {
    id: r?.emailer_template?.id || templateId,
    subiect: r?.emailer_template?.subject || subiect,
  }
}

/**
 * HTML-ul primit de la Apollo, adus înapoi la text simplu ca să-l poți edita
 * într-o casetă normală. Paragrafele și <br>-urile devin rânduri.
 *
 * Perechea lui `textInHtml`: text → HTML → text dă înapoi exact ce ai scris.
 */
export function htmlInText(html) {
  if (!html) return ''
