...
export async function getStudentLearningAccess(studentId) {
  const [student, latestPayment, manualAccesses] = await Promise.all([
  ...
  const hasAnyManualAccess = manualModuleIds.size > 0
  ...
  function canAccessLesson(lesson) {
    ...
    if (subscriptionActive) return true
    return manualModuleIds.has(lesson.moduleId)
  }

  let lockReason = null
  if (!isActive) lockReason = 'INACTIVE'
  else if (!isSuper && !subscriptionActive && manualModuleIds.size === 0) lockReason = 'PAYMENT_REQUIRED'
