...
/**
 * Memento de recontactare pentru lead-urile web (firmele fără site).
 *
 * Trimis de cron-ul /api/cron/leaduri-followup. Un singur mesaj pentru toate
 * firmele scadente, nu câte unul de fiecare — altfel dimineața primești
 * treizeci de notificări și le ignori pe toate.
 *
 * @param {Array<{denumire, telefon, scor, oras, nextFollowUpAt, calitateSite, restant}>} leaduri
 */
export async function notifyLeaduriFollowUp(leaduri) {
  if (!leaduri?.length) return false
  ...
  const rand = (l) =>
    `${l.restant ? '🔴' : '🟠'} <b>${l.denumire}</b> — ${l.telefon || 'fără telefon'}\n` +
    `   ${ora(l.nextFollowUpAt)} · scor ${l.scor} · ${l.oras || '—'}`

  let msg = `⏰ <b>DE RECONTACTAT — LEADURI WEB</b>\n`

  if (restante.length) {
    msg += `\n<b>Restante (${restante.length}):</b>\n${restante.map(rand).join('\n')}\n`
  }
  if (azi.length) {
    msg += `\n<b>Azi (${azi.length}):</b>\n${azi.map(rand).join('\n')}\n`
  }
  ...
  return sendTelegramMessage(
    ...
    msg,
    'HTML',
    process.env.TELEGRAM_ADMIN_THREAD_ID
  )
}
