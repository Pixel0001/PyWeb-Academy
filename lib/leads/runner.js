...
const DURATA_LOCK_MS = 3 * 60 * 1000
...
async function salveazaFirme(firme, runId) {
  ...
  for (const u of deActualizat) {
    await prisma.webLead.update({ where: { id: u.id }, data: u.date })
  }
...
async function pasCautare(rulare, apiKey) {
...
async function pasVerificare(rulare) {
  ...
  const rezultate = await verificaSiteuri(lot)

  for (const lead of lot) {
  ...
  loguri = adaugaLog(loguri, `🔎 ${lot.length} site-uri verificate, ${ramase} rămase.`)
...
async function pasPitch(rulare) {
  ...
  const { pitchuri, cost, furnizor, erori, statistici } = await genereazaPitchuri(lot)
  ...
  for (const lead of lot) {
    ...
    const pitch = pitchuri.get(lead.placeId) || pitchSablon(lead)
    await prisma.webLead.update({ where: { id: lead.id }, data: { pitch } })
  }
...
export async function executaPas(runId) {
  ...
  try {
    ...
    if (rulare.faza === 'SEARCH') {
      actualizari = await pasCautare(rulare, apiKey)
    } else if (rulare.faza === 'CHECK') {
      actualizari = await pasVerificare(rulare)
    } else if (rulare.faza === 'PITCH') {
      const rezultat = await pasPitch(rulare)
