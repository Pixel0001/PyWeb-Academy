  ...
  async () => {
    const [allStudents, allSubmissions, allBonusPoints] = await Promise.all([
      prisma.student.findMany({
        ...
        select: {
          ...
          equipped: {
            where: { type: 'TITLE' },
            select: { cosmetic: { select: { name: true, imageUrl: true, rarity: true } } },
            take: 1,
          },
        },
      }),
    ...
    return allStudents
      .map(s => ({
        ...
        fullName: s.fullName,
        xp: xpMap.get(s.id) ?? 0,
        titleName:   s.equipped?.[0]?.cosmetic?.name || null,
        titleIcon:   s.equipped?.[0]?.cosmetic?.imageUrl || null,
        titleRarity: s.equipped?.[0]?.cosmetic?.rarity || null,
        titleEffect: s.equipped?.[0]?.cosmetic?.cssPayload?.titleEffect || null,
      }))
      .sort((a, b) => b.xp - a.xp)
  },
  ['leaderboard-ranking'],
