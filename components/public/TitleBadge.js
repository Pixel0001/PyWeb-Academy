...
/**
 * TitleBadge � titluri in stil Brawl Stars.
 * Text 3D cu stroke + shadow-uri stratificate + animatii SVG (fulgere, foc, particule).
 */
...
export const TITLE_EFFECTS = [
  ...
  { value: 'ice',       label: '❄️ Cristal de Gheata' },
...
function LightningBadge({ label }) {
  ...
  return (
    <span style={{
      ...
      <svg width="10" height="18" viewBox="0 0 10 18" style={boltStyle('-0.3s')}>
      ...
      <svg width="10" height="18" viewBox="0 0 10 18" style={boltStyle('-1.1s')}>
        <polygon points="7,0 3,8 6,8 3,18 10,6 6,6" fill="#facc15"/>
        <polygon points="7,0 3,8 6,8 3,18 10,6 6,6" fill="#fff" opacity="0.4"/>
      </svg>
    </span>
        ...
        {flames.map((f,i) => (
          <g key={i}>
            <ellipse cx={f.x} cy={12} rx={3} ry={5} fill={f.color}
              style={{animation:`tb-fire-rise ${f.dur} ease-out infinite`,animationDelay:f.delay,
                filter:`drop-shadow(0 0 3px ${f.color})`}} />
...
function NeonBadge({ label }) {
  return (
    <span style={{
      ...
      <span style={{position:'absolute',top:0,left:0,right:0,height:'40%',
