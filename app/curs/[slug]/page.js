...
export default function CourseDetailPage() {
    ...
    const fetchData = async () => {
      try {
        const [courseRes, allRes] = await Promise.all([
          fetch(`/api/public/courses/${params.slug}`),
          fetch('/api/public/courses')
        ])
        if (!courseRes.ok) { setError(courseRes.status === 404 ? 'Cursul nu a fost găsit' : 'Eroare la încărcare'); return }
        const [courseData, allData] = await Promise.all([courseRes.json(), allRes.json()])
        setCourse(courseData)
        setAllCourses(Array.isArray(allData) ? allData.filter(c => c.slug !== params.slug) : [])
  ...
  if (loading) return (
    <>
      <Navbar forceOpaque />
      <main style={{ minHeight: '100vh', backgroundColor: 'var(--bg-page)', paddingTop: '5.5rem' }}>
        <div style={{ maxWidth: 1140, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
            <div style={{ aspectRatio: '4/3', backgroundColor: 'var(--border-light)', borderRadius: '1.5rem', opacity: 0.5 }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', paddingTop: '0.5rem' }}>
              {[50, 36, 24, 24, 90].map((h, i) => (
                <div key={i} style={{ height: h, backgroundColor: 'var(--border-light)', borderRadius: '0.5rem', opacity: 0.5, width: i === 2 ? '55%' : '100%' }} />
              ))}
            </div>
          </div>
        </div>
    ...
    <>
      ...
      <main style={{ minHeight: '100vh', backgroundColor: 'var(--bg-page)', paddingTop: '5rem' }}>
        <div style={{ maxWidth: 500, margin: '0 auto', padding: '5rem 1.5rem', textAlign: 'center' }}>
          ...
          <a href="/#cursuri" style={{ color: 'var(--color-primary)', fontWeight: 600, textDecoration: 'none' }}>← Înapoi la cursuri</a>
        </div>
      </main>
      <Footer />
    </>
